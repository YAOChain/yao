...
import (
	...
	"github.com/YAOChain/yao/client/lcd"
	"github.com/YAOChain/yao/codec"
	crkeys "github.com/YAOChain/yao/crypto/keys"
	...
	authrest "github.com/YAOChain/yao/x/auth/client/rest"
	...
	tmcfg "github.com/YAOChain/yao/core/config"
...
func InitializeLCD(nValidators int, initAddrs []sdk.AccAddress, minting bool, portExt ...string) (
	...
	db := dbm.NewMemDB()
	app := gapp.NewYaoApp(logger, db, nil, true, 0, baseapp.SetPruning(store.PruneNothing))
	cdc = gapp.MakeCodec()
	...
	if len(portExt) == 0 {
		listenAddr, port, err = node.FreeTCPAddr()
		if err != nil {
			return
		}
	} else {
...
func startTM(
	...
	privVal tmtypes.PrivValidator, app *gapp.YaoApp,
) (*nm.Node, error) {
...
func GetConfig() (*tmcfg.Config, error) {
	...
	tmAddr, _, err := node.FreeTCPAddr()
	if err != nil {
		return nil, err
	}

	rcpAddr, _, err := node.FreeTCPAddr()
	if err != nil {
		return nil, err
	}

	grpcAddr, _, err := node.FreeTCPAddr()
	if err != nil {
		return nil, err
	}
