...
import (
	...
	"github.com/YAOChain/yao/crypto/keys/mintkey"
	...
	dclcommon "github.com/YAOChain/yao/x/distribution/client/common"
	distrrest "github.com/YAOChain/yao/x/distribution/client/rest"
	disttypes "github.com/YAOChain/yao/x/distribution/types"
...
func TestAccountBalanceQuery(t *testing.T) {
	...
	res, body := Request(t, port, "GET", fmt.Sprintf("/auth/accounts/%s", someFakeAddr), nil)
	require.Equal(t, http.StatusOK, res.StatusCode, body)
	require.Contains(t, body, `"type":"yao/Account"`)
