...
func NewFixtures(t *testing.T) *Fixtures {
	...
	servAddr, port, err := node.FreeTCPAddr()
	require.NoError(t, err)

	p2pAddr, _, err := node.FreeTCPAddr()
	require.NoError(t, err)
	...
	return &Fixtures{
		...
		BuildDir:      buildDir,
		RootDir:       tmpDir,
		GaiadBinary:   filepath.Join(buildDir, "demo"),
		GaiacliBinary: filepath.Join(buildDir, "democli"),
		GaiadHome:     filepath.Join(tmpDir, ".demo"),
		GaiacliHome:   filepath.Join(tmpDir, ".democli"),
		RPCAddr:       servAddr,
		P2PAddr:       p2pAddr,
		Port:          port,
	}
