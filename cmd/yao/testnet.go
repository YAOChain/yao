...
import (
	...
	tmconfig "github.com/YAOChain/yao/core/config"
	...
	"github.com/YAOChain/yao/client/keys"
	"github.com/YAOChain/yao/codec"
	srvconfig "github.com/YAOChain/yao/node/config"
	...
	genutiltypes "github.com/YAOChain/yao/x/genutil/types"
...
func testnetCmd(ctx *node.Context, cdc *codec.Codec,
	mbm module.BasicManager, genAccIterator genutiltypes.GenesisAccountsIterator,
) *cobra.Command {

	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a demo testnet",
...
Example:
	demo testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			minGasPrices := viper.GetString(node.FlagMinGasPrices)
	...
	cmd.Flags().String(flagNodeDaemonHome, "demo",
		"Home directory of the node's daemon configuration")
	cmd.Flags().String(flagNodeCLIHome, "democli",
	...
	cmd.Flags().String(
		node.FlagMinGasPrices, fmt.Sprintf("0.000006%s", sdk.DefaultBondDenom),
...
func InitTestnet(cmd *cobra.Command, config *tmconfig.Config, cdc *codec.Codec,
	...
	for i := 0; i < numValidators; i++ {
		...
		addr, secret, err := node.GenerateSaveCoinKey(clientDir, nodeDirName, keyPass, true)
		if err != nil {
			_ = os.RemoveAll(outputDir)
			return err
		}
		...
		gaiaConfigFilePath := filepath.Join(nodeDir, "config/demo.toml")
